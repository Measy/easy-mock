<template>
    <div class="em-proj-detail">
      ...
      <Modal
        ...
        :title="$t('p.detail.add.modalTitle')"
        :loading="optCase.loading"
        @on-ok="newApiCase">
        <Form ref="optCase" :model="optCase" :label-width="100">
            <FormItem :label="$t('p.detail.add.apiCaseName')" prop="caseName">
            ...
            <FormItem label="copy from" prop="srcCase">
                <i-select v-model="optCase.srcCase" key="select1">
                    <Option v-for="casename in project.cases" :value="casename" :key="casename">{{ casename }}</Option>
                </i-select>
            </FormItem>
        </Form>
      </Modal>
      ...
      <transition name="fade" mode="out-in">
        ...
        <div
          ...
          <div class="em-proj-detail__switcher">
            <ul>
              <li @click="optCase.show = true" v-shortkey="['ctrl', 'a']" @shortkey="optCase.show = true">
                <Icon type="plus-round"></Icon> {{$t('p.detail.add.action')}}
              </li>
              <li @click="deleteCurCase" v-shortkey="['ctrl', 'd']" @shortkey="deleteCurCase">
                <Icon type="trash-a"></Icon> {{$t('p.detail.delete.action')}}
              </li>
          ...
          <div class="em-proj-detail__cases">
              <h2><Icon type="fork-repo"></Icon> {{$t('p.detail.chose.action')}}</h2>
              <i-select v-model="currentCase" key="select2">
                <Option v-for="casename in project.cases" :value="casename" :label="casename" :key="casename"></Option>
              </i-select>
          </div>
          <Table
            border
    ...
    computed: {
      ...
      currentCase: {
        ...
        set: async function (val) {
          await this.$store.dispatch('mock/CHOSE_CASE', {caseName: val, projectId: this.project._id}).then(res => {
            if (res.data.success) this.$Message.success(`${this.$t('p.detail.chose.success')}${val}`)
          })
        }
      },
      baseUrl () {
    ...
    methods: {
      ...
      newApiCase () { // 这里新建case之后会默认选中新建的场景，实际上展示了新建的场景，但是select组件的截流器设置bug，导致选中的是默认第一个，暂时先在源码中修改，后面再起一个pr
        ...
        if (!this.optCase.caseName) {
          this.$Message.error(this.$t('p.detail.add.noneApiCaseNameErr'))
        ...
        this.$store.dispatch('mock/COPY_CASE', {
          ...
          if (res.data.success) {
            this.$Message.success(`${this.optCase.caseName} ${this.$t('p.detail.add.success')}`)
            this.optCase = {
              show: false,
              loading: true,
              caseName: '',
      ...
      deleteCurCase () {
        if (this.currentCase === 'default') {
          this.$Message.error(this.$t('p.detail.delete.deleteDefaultErr'))
          return
        }
        this.$Modal.confirm({
          title: this.$t('p.detail.delete.confirmTitle'),
          content: `${this.$t('p.detail.delete.confirmContent')}${this.currentCase}`,
          onOk: () => {
            this.$store.dispatch('mock/DELETE_CASE', {
              ...
              if (res.data.success) {
                this.$Message.success(`${this.currentCase}: ${this.$t('p.detail.delete.success')}`)
              }
            })
          }
        })
