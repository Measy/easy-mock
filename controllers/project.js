...
exports.createBySwagger = function * () {
  ...
  const swaggerInfo = yield swagger.getSwaggerInfo({url: swaggerUrl})
  ...
  // 基于 swagger 创建或更新 mock
  try {
    yield swagger.create(project)
  } catch (err) {
    this.log.error(
      { req: this.req, err },
      '  --> %s %s, 基于 Swagger 创建或更新 Mock 发生异常',
      this.request.method,
      this.request.originalUrl
    )
  }
