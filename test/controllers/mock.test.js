...
describe('test/controllers/mock.test.js', () => {
    ...
    test('场景不存在', async () => {
      const res = await request('/api/mock/create', 'post')
        .send({
          ...
          method: 'get',
          description: 'demo',
          api_case: 'notexistcase'
        })

      expect(res.body.message).toBe('场景不存在')
    })

    test('创建个人项目接口,并复制场景', async () => {
      ...
      expect(project.cases[0]).toBe('default')
      expect(project.cases[1]).toBe('newCase')
    })
  ...
  describe('getMockAPI', () => {
    function getMockURL (url, proj = project) {
      return `/mock/${proj._id}/${user._id}${proj.url}${url}`
    }
    ...
    test('404', async () => {
      ...
      await request(`/mock/${project._id}/${user._id}/ttest`).expect(404)
    })

    test('接口请求频率太快，已被限制访问', async () => {
      const res = await request(`/mock/222222222233333333331212/${user._id}/user`)
    ...
    test('/mock', async () => {
      ...
      const res = await request(`/mock/${project._id}/${user._id}/mock`)
      expect(res.body.success).toBe(true)
    })
  })
  ...
  describe('setCurrent', () => {
    let api0, api1
    test('激活新场景', async () => {
      const api0Res = await request('/api/mock/create', 'post')
        .send({
          project_id: project._id,
      ...
      let res = await request(`/mock/${project._id}/${user._id}/mockforsetcurrent`)
      expect(res.body.currentNo).toBe(0)

      res = await request(`/api/mock/${api1._id}/current`, 'put')
      expect(res.body.success).toBe(true)

      res = await request(`/mock/${project._id}/${user._id}/mockforsetcurrent`)
      expect(res.body.currentNo).toBe(1)
    })

    test('没有权限', async () => {
      const res = await request(`/api/mock/${api0._id}/current`, 'put', soucheUser.token)
      expect(res.body.message).toBe('无权限操作')
      expect(res.body.success).toBe(false)
    })

    test('没有权限', async () => {
      const res = await request(`/api/mock/123123213123/current`, 'put', soucheUser.token)
      expect(res.body.message).toBe('接口不存在')
      expect(res.body.success).toBe(false)
    })

    test('删除了当前处于激活接口状态的接口,如果有剩余未激活的接口，自动激活剩余的某个接口', async () => {
      let res = await request(`/mock/${project._id}/${user._id}/mockforsetcurrent`)
      expect(res.body.currentNo).toBe(1)

      res = await request('/api/mock/delete', 'post')
        .send({ project_id: project._id, ids: [ api1 ] })
      expect(res.body.success).toBe(true)
